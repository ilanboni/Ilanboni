...
export default function ClientInteractionsHistory({ clientId, days = 30 }: ClientInteractionsHistoryProps) {
  const { data: interactions, isLoading } = useQuery<ClientInteraction[]>({
    queryKey: [`/api/clients/${clientId}/interactions`, { since: `${days}d` }],
    queryFn: async () => {
      const response = await fetch(`/api/clients/${clientId}/interactions?since=${days}d`);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Cronologia Invii (ultimi {days} giorni)
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  if (!interactions || interactions.length === 0) {
    return (
      <Card>
        ...
        <CardContent>
          <div className="text-center text-gray-500 py-8">
            Nessuna interazione negli ultimi {days} giorni
          </div>
        </CardContent>
      </Card>
    );
            ...
            <div className="flex items-start gap-3">
              <div className="flex-1 space-y-2">
                ...
                {interaction.property && (
                  <Link href={`/properties/${interaction.property.id}`}>
                    <div className="flex items-start gap-2 cursor-pointer hover:text-blue-600">
                      <MapPin className="h-4 w-4 mt-0.5 flex-shrink-0" />
                      <span className="text-sm font-medium">{interaction.property.address}</span>
                    </div>
                  </Link>
                )}
