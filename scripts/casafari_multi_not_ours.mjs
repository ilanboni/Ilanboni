...
// e non richiede più owner_type='agency' (cattura cluster misti privato+agenzia)
...
// --- HTTP helpers ---
async function httpJson(url,opts={}){ const r=await fetch(url,opts); if(r.status===204) return null; if(!r.ok) throw new Error(`${opts.method||"GET"} ${url} → ${r.status} ${await r.text().catch(()=> "")}`); return r.json(); }
...
async function fetchPortfolioIndex(){
  ...
  }catch(e){ console.warn("⚠️  Nessun indice portafoglio:", String(e.message||e)); return {urls:new Set(),ids:new Set(),addrs:new Set(),ok:false}; }
}
...
async function main(){
  console.log(`🔗 Casafari → multi NON nostri entro ${RADIUS_KM}km (fallback su "Milano")`);
  ...
  for (const a of alerts){
    ...
    // 4) escludi se già nel portafoglio
    ...
    for (let i=0;i<mapped.length;i+=50){
      ...
      kept += sent;
      console.log(`  ✅ Importati ${sent} (batch ${i/50+1}) — es: ${slice.slice(0,2).map(s=>s.url||s.title).join(" | ")}`);
      await sleep(350);
    }
    await sleep(600);
  }

  console.log(`\n📦 Letti: ${read} | Import utili: ${kept}`);
  if(kept>0){ console.log("⚙️  Match…"); await triggerMatch(); console.log("✅ Match ok."); }
  else { console.log("ℹ️ Nessun nuovo multi."); }
  console.log("🏁 Fine.");
}
