...
**UI/UX Decisions:**
- Frontend built with React, TypeScript, Vite, Tailwind CSS, shadcn/ui, and Wouter for routing.
- Mobile-responsive design.
- Interactive maps powered by Leaflet, with geocoding via Nominatim.
- Card-based interface with color-coded visual cues for property status.
- Support for viewing all matching competitor properties with accurate geographic filtering using FeatureCollection for multi-zone buyer searches.
- Automatic zone geocoding system using Nominatim API to visualize buyer search areas.
...
- **Data Management**: Intelligent bucketing algorithm for property deduplication, multi-signal owner classification, persistent match caching with 15-minute TTL for optimized performance (shared properties cached in database, private properties computed on-the-fly), automatic cache pre-computation after daily property imports, and an asynchronous job queue system for long-running operations with auto-recovery and error handling.
- **Client & Property Matching**: Dual favorites system (global and per-client), per-client ignore functionality for properties with bulk operations (multi-select mode for "Possibili Immobili" section allowing bulk ignore of multiple properties), and bidirectional matching views showing "Potenziali Interessati" (matching clients for a property) and "Possibili Immobili" (matching properties for a client). Intelligent matching tolerances: size range -20% to +150% of minSize (max 2.5x), 500m radius for Point searches, 50m tolerance for Polygon boundaries. Properties without GPS coordinates are excluded when buyer has searchArea.
...
- **Auto-Import**: Zero-friction Idealista import with auto-save, 5-layer extraction fallback system (including Apify for complex cases), and Casafari saved properties import via Apify.
