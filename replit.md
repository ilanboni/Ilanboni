...
- **Feature Specifications**:
    ...
    - **Automated Buyer Scraping**: Weekly automatic scraping scheduler (Martedì e Venerdì alle 2:00 AM) that finds ALL relevant properties for active buyers using Apify. Each buyer with defined criteria gets an automatic scraping job with proper tracking (queued → running → completed/failed). The system prevents duplicate runs within 24 hours by checking for recently completed jobs. **Current Limitations**: Zone filtering is not yet supported during scraping - properties are fetched by type/size/price only and zone filtering must be applied manually after retrieval. Background job processing with 5-second spacing between buyers to avoid overwhelming Apify API. Uses node-cron for reliable scheduling.
